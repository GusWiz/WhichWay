...
describe('PreferenceModal', () => {
  ...
  it('renders modal content', () => {
    ...
    expect(screen.getByLabelText(/Transportation Preference/i)).toBeInTheDocument();
  });
  ...
  it('fills and submits form', async () => {
    ...
    fireEvent.change(screen.getByLabelText(/Cuisine Preference/i), { target: { value: 'asian' } });
    fireEvent.change(screen.getByLabelText(/Activity Type/i), { target: { value: 'entertainment' } });
    fireEvent.change(screen.getByLabelText(/Budget Preference/i), { target: { value: 'medium' } });
    fireEvent.change(screen.getByLabelText(/Transportation Preference/i), { target: { value: 'public' } });

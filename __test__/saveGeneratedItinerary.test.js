...
describe('saveGeneratedItinerary Function', () => {
    ...
    const mockExistingItineraryRef = { id: mockExistingItineraryId, path: `Itineraries/${mockExistingItineraryId}` };
    ...
    beforeEach(() => {
        ...
        doc.mockImplementation((db, collectionName, id) => {
            ...
            if (collectionName === 'Itineraries') return { id: id || 'mockGeneratedItineraryId', path: `Itineraries/${id || 'mockGeneratedItineraryId'}` };
            if (collectionName === 'Users') return { id: id || 'testUser123', path: `Users/${id || 'testUser123'}` }; // Mock user ref creation
            return { id: id || 'mockGenericId', path: `${collectionName}/${id || 'mockGenericId'}` };
        });
        addDoc.mockResolvedValue({ id: 'mockGeneratedItineraryId' }); // Default addDoc success
        getDoc.mockResolvedValue({ exists: () => true, data: () => ({ userId: 'user123' }) }); // Default getDoc success (with userId)
        updateDoc.mockResolvedValue(); // Default updateDoc success
        setDoc.mockResolvedValue(); // Default setDoc success
        collection.mockImplementation((db, collectionName) => ({ path: collectionName })); // Default collection mock
    });
    ...
    it('should handle Firestore errors during addDoc', async () => {
        ...
        getDoc.mockResolvedValue({ exists: () => true, data: () => ({ userId: 'user123' }) });
